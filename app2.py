...
#@st.cache_data is a decorator in Streamlit used to cache the results of a function, preventing expensive computations (like file loading) from running multiple times unnecessarily
@st.cache_data 
def load_data(path):
    df = pd.read_csv(path)
    return df
